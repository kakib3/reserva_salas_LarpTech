...
def verificar_conflito(id_sala, data, hora_inicio, hora_fim): # Essa função só verifica conflitos.
    ...
    mesma_sala_e_data = (reservas["idSala"] == id_sala) & (reservas["data"] == data) & (reservas["status"] == "Confirmado")
...
def cancelar_reserva(id_usuario, id_sala, data, hora_inicio, hora_fim):
    arquivo = DATA_DIR / "reservas.csv"

    df = pd.read_csv(arquivo)

    filtro = (
        (df["idUser"] == id_usuario)
        & (df["idSala"] == id_sala )
        & (df["data"] == data)
        & (df["horaInicio"] == hora_inicio)
        & (df["horaFim"] == hora_fim)
        ) 
...
def alterar_reserva(id_usuario, id_sala, data, hora_inicio, hora_fim, nova_sala, nova_data, novo_inicio, novo_fim):
    arquivo = DATA_DIR / "reservas.csv"
    df = pd.read_csv(arquivo)
